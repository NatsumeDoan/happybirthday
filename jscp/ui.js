...
S.UI = (function () {
  ...
  videoEl.setAttribute('playsinline', ''); // playsinline cho iOS
  // Nếu muốn ẩn controls thì không set `controls`

  // Thêm source
  const source = document.createElement('source');
  source.src = './image/video.mp4'; // kiểm tra đường dẫn
  source.type = 'video/mp4';
  videoEl.appendChild(source);

  // Append video vào container
  videoContainer.appendChild(videoEl);

  // Tùy chọn: cố gắng play (một số trình duyệt yêu cầu gọi play() sau khi append)
  videoEl.play().catch((err) => {
    // Nếu trình duyệt chặn autoplay vì chính sách (thường do chưa muted), log lỗi nhưng không crash
